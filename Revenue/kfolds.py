...
data = data.drop(columns=["DayOfWeek","Customers","Open","Promo","StateHoliday","SchoolHoliday", "Store"])
...
tscv = TimeSeriesSplit(n_splits=N_SPLITS, max_train_size=900000, test_size=100000)
results = []
best_model = None
best_loss = float('inf')
...
plt.plot(best_train_idx, data['Sales'][best_train_idx], label='Train')
plt.plot(best_test_idx[:-WINDOW-1], data['Sales'][best_test_idx][:-WINDOW-1], label='Test')
plt.plot(best_test_idx[:-WINDOW-1], best_predictions, label='Predictions')
plt.title(f'{MODEL} Forecast of Sales')
plt.xlabel('Time')
plt.ylabel('Sales')
plt.legend()
plt.show()
...
# Metrics -------------------------------------------------------------------------------
mse = mean_squared_error(data['Sales'][best_test_idx].values[:-WINDOW-1], best_predictions).round(4)
mae = mean_absolute_error(data['Sales'][best_test_idx].values[:-WINDOW-1], best_predictions).round(4)
