...
tscv = TimeSeriesSplit(n_splits=N_SPLITS, max_train_size=2070, test_size=230)
results = []
best_model = None
best_loss = float('inf')
...
plt.plot(best_train_idx, data['Close'][best_train_idx], label='Train')
plt.plot(best_test_idx[:-WINDOW-1], data['Close'][best_test_idx][:-WINDOW-1], label='Test')
...
# Metrics -------------------------------------------------------------------------------
mse = mean_squared_error(data['Close'][best_test_idx].values[:-WINDOW-1], best_predictions).round(4)
mae = mean_absolute_error(data['Close'][best_test_idx].values[:-WINDOW-1], best_predictions).round(4)
