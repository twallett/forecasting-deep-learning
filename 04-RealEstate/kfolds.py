...
tscv = TimeSeriesSplit(n_splits=N_SPLITS, max_train_size=24300, test_size=2700)
results = []
best_model = None
best_loss = float('inf')
...
plt.plot(best_train_idx, data['Price'][best_train_idx], label='Train')
plt.plot(best_test_idx[:-WINDOW-1], data['Price'][best_test_idx][:-WINDOW-1], label='Test')
...
# Metrics -------------------------------------------------------------------------------
mse = mean_squared_error(data['Price'][best_test_idx].values[:-WINDOW-1], best_predictions).round(4)
mae = mean_absolute_error(data['Price'][best_test_idx].values[:-WINDOW-1], best_predictions).round(4)
